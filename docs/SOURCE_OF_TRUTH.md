...
### 2026-06-15 — Clock In/Out: Eastern (EST/EDT) canonical time + per-viewer local aside
- **All punch times now display in Eastern Time (America/New_York, DST-aware)** as the canonical company time — independent of where the viewer is. When the viewer's own timezone differs, a small muted **"· 6:18 PM local"** aside appears beside the Eastern time (ET viewers see no extra text). Touches the live Punch clock, "Clocked in at", Today list, weekly Timecard, Who's In, and the Manage records table; each tab shows a "🌐 Eastern Time · EDT" anchor badge.
- **Service (`TimeClockService.ts`)**: `COMPANY_TZ`, ET-anchored `fmtTime`/`fmtDateTime`, plus `etAbbrev`, `fmtTimeLocal`, `localTimeIfDifferent`, `viewerOffEastern`. **View (`TimeClockView.tsx`)**: mount-safe `useOffEastern()` hook (no SSR hydration mismatch — ET primary is deterministic), `<ZoneBadge>` / `<LocalAside>` / `<LocalRangeAside>`. Manual edit/add `datetime-local` inputs read the editor's *local* clock, so an amber hint shows there **only for off-Eastern editors**. Pure formatting layer (absolute `Timestamp`s already stored — no backfill). Details + known day-bucketing caveat in §8.3.
...
- ✅ **DONE (2026-06-15): Timezone handling — Eastern is canonical company time.** All Clock In/Out times now render anchored to **America/New_York (EST/EDT, DST-aware)** regardless of where the viewer sits, and when the viewer is in a *different* timezone the UI adds a small muted **"· 6:18 PM local"** aside next to the Eastern time. Implementation (pure formatting layer — `time_entries` already store absolute `Timestamp`s, no backfill):
  - **`TimeClockService.ts`**: `COMPANY_TZ='America/New_York'`; `fmtTime`/`fmtDateTime` redefined to format with explicit `timeZone:COMPANY_TZ` (so they're SSR-deterministic and identical for every viewer); new `etAbbrev(ts)` (→ "EST"/"EDT" via `Intl…timeZoneName:'short'`), `fmtTimeLocal(ts)` (viewer-local), `localTimeIfDifferent(ts)` (local string only when it differs from ET), and `viewerOffEastern()` (compares the current instant rendered both ways — captures DST).
  - **`TimeClockView.tsx`**: `useOffEastern()` hook resolves the browser tz **after mount** (so the local asides never cause an SSR hydration mismatch — the ET primary renders first and is deterministic). Reusable `<ZoneBadge>` ("🌐 Eastern Time · EDT"), `<LocalAside>` (single time), and `<LocalRangeAside>` (in→out pair) render the muted local time **only for off-Eastern viewers** — ET viewers (the majority) see zero extra clutter. Wired into the live Punch clock (ET big clock + "your local time" line), the "Clocked in at" line, Today list, weekly Timecard rows, Who's In "since", and the Manage records table (per-cell local line). The manual **edit/add `datetime-local` inputs** read the editor's *local* wall clock, so an amber "🌐 Times you enter are read in your local timezone, not Eastern" hint now shows in AddEntryForm + RequestEditModal **only when the editor is off-Eastern** (ET editors see nothing). *Known limitation:* day/week bucketing (`startOfWeek`, `dayKey`, `isSameDay`) still uses the viewer's local calendar day, so a near-midnight punch could land on the adjacent day for an off-ET viewer — acceptable for an ET-based org; revisit with ET-based bucketing if remote staff grow.
