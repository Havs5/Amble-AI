/**
 * PostDetailModal — full-post reader that takes over the entire right panel
 * (covering the page greeting), leaving the icon sidebar visible. Opened by
 * clicking any post card.
 */
...
export function PostDetailModal({
  ...
  return (
    <>
      <div
        className="fixed inset-y-0 right-0 left-0 lg:left-[68px] z-[100] flex flex-col bg-slate-50 dark:bg-slate-950"
        style={{ animation: 'fade-in-up 0.18s ease-out both' }}
      >
        {/* ── Top bar ── */}
        <div className="shrink-0 flex items-center justify-between gap-3 h-14 px-4 sm:px-6 border-b border-slate-200 dark:border-slate-800 bg-white/85 dark:bg-slate-900/85 backdrop-blur">
          <button
            onClick={onClose}
          ...
          {isAdmin && (
            <div className="flex items-center gap-1.5">
              ...
              <button
                onClick={onClose}
                className="ml-1 p-1.5 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                aria-label="Close"
              >
                <X size={18} />
              </button>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin">
          ...
          <div className="relative h-44 sm:h-60">
            {hasImage ? (
              <>
                ...
                <button
                  onClick={() => setZoomed(true)}
                  className="absolute top-3 right-3 z-10 inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-black/35 text-white/90 hover:text-white hover:bg-black/55 backdrop-blur-sm text-[11px] font-medium transition-colors"
                  title="Zoom image"
                >
                  <ZoomIn size={13} /> Zoom
                </button>
              ...
              <div className={`absolute inset-0 bg-gradient-to-br ${gradient}`}>
                <div className="absolute inset-0 flex items-center justify-center">
                  <Building2 size={52} className="text-white/20" />
                </div>
              </div>
            )}
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent" />
            <div className="absolute bottom-3 left-0 right-0 px-5 sm:px-8">
          ...
          <div className="max-w-3xl mx-auto px-5 sm:px-8 py-6 sm:py-8">
            ...
            <div className="flex items-center gap-2 mt-3 text-sm text-slate-500 dark:text-slate-400">
              ...
              <span className="flex items-center gap-1.5"><Clock size={14} /> {formatDateLong(post.publishedAt)}</span>
            </div>

            {post.summary && (
              <p className="mt-5 text-base font-medium text-slate-600 dark:text-slate-300 leading-relaxed">
                {post.summary}
              </p>
            )}

            <div className="mt-5 text-[15px] text-slate-700 dark:text-slate-200 whitespace-pre-wrap leading-7">
              {post.body}
            </div>

            {post.reactions && Object.values(post.reactions).some((n) => (n || 0) > 0) && (
              <div className="mt-7">
                <div className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-2">Acknowledgements</div>
                <ReactionsBar reactions={post.reactions} />
              </div>
            )}

            {post.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-7 pt-5 border-t border-slate-200 dark:border-slate-800">
                {post.tags.map((t) => (
