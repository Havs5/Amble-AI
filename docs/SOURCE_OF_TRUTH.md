...
### 2026-06-15 — Clock In/Out: Eastern (EST/EDT) canonical time
- **All punch times now display in Eastern Time (America/New_York, DST-aware)** as the canonical company time — identical for every viewer regardless of their own timezone. `TimeClockService.fmtTime`/`fmtDateTime` pass an explicit `timeZone: COMPANY_TZ` (also makes them SSR-deterministic). Absolute `Timestamp`s already stored, so this is a pure display change — no backfill.
- **Per-viewer local-time aside removed at owner's request.** An earlier pass also showed each viewer's *local* time as a muted "· 6:18 PM local" aside plus a "🌐 Eastern Time · EDT" badge; the owner didn't want it, so the badge, all local asides (Punch clock, Today, Timecard, Who's In, Manage records), and the off-Eastern input hints were removed. Times are now Eastern-only, clean.
...
- ✅ **DONE (2026-06-15): Timezone handling — Eastern is canonical company time.** All Clock In/Out times render anchored to **America/New_York (EST/EDT, DST-aware)** and are identical for every viewer. Pure display layer — `time_entries` already store absolute `Timestamp`s, no backfill.
  ...
  - **Per-viewer local aside — built then removed at owner's request.** The first pass added a "🌐 Eastern Time · EDT" `<ZoneBadge>` plus muted "· 6:18 PM local" asides (and off-Eastern input hints) so a remote teammate could see both times. The owner decided against it ("delete where it says eastern time" / "delete the local hour in the recorded time"), so all of it — badge, `LocalAside`/`LocalRangeAside`, the Punch "your local time" line, Who's In local line, Manage per-cell local lines, and the AddEntry/RequestEdit amber hints — was removed. If a per-viewer local time is ever wanted again, re-add a mount-resolved `viewerOffEastern()` gate (kept out of an SSR render to avoid hydration mismatch).
