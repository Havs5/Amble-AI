...
> **Provider note (2026-06-14):** **Chat runs on Vertex AI** — `functions/src/routes/chat.js` uses `@google/genai` in Vertex mode (`vertexai:true`, **ADC** auth via the runtime service account with `roles/aiplatform.user`) on the **`global`** endpoint. Models: **`gemini-3-flash-preview`** (fast) and **`gemini-3.1-pro-preview`** (pro/reasoning) — the latest Gemini on Vertex (`normalizeModel` collapses any Gemini selection to these two). These are *preview* IDs that can rotate (e.g. `gemini-3-pro-preview` was retired), so the handler **falls back to OpenAI (`gpt-5-mini`) on any Gemini error** — chat never hard-fails. ⚠️ Gemini 3 is on the **global** endpoint, not regional `us-central1`. **Also on Vertex now:** **image** (Imagen 4 — `image.js`, `imagen-4.0-generate-001`, **regional `us-central1`**) and **video-analysis** (`videoAnalyze.js`, `gemini-2.5-flash`, passes the Storage video as a `gs://` URI — no more Developer-API file upload). **Still on the Gemini Developer API (API key):** **Veo** video generation (`video.js` + dev `veo/route.ts`) — left as-is because its Vertex output handling (inline bytes vs GCS URI) is hard to verify in one session and **Sora remains the working video path**; plus the dev-only `src/app/api/chat/route.ts`. The browser **Live Studio was removed**. See [SOURCE_OF_TRUTH.md §8](./SOURCE_OF_TRUTH.md#8-open-items--next-session). ⚠️ Imagen/Veo are **regional (us-central1)**; Gemini chat is **global** — the Vertex client `location` differs per feature.
...
> ℹ️ The legacy `src/services/knowledge/*` hybrid pipeline (client/dev) and the **dev** Next.js routes still exist, but **production now runs the §11a vector pipeline** in `functions/src/routes/*.js`.

### 11a. KB Retrieval — production vector RAG (shipped 2026-06-14)

The Amble AI chat answer path (`/api/chat → handleChat`) is now **semantic vector RAG with hybrid fusion, reranking, grounded generation, and abstention** — with the old live-Drive keyword search kept only as a cold-start fallback.
...
flowchart TD
    ...
    FLOOR -->|yes| INJ["Inject top chunks w/ [n] citations + [dept]"]
    FLOOR -->|no| FB["Fallback: live-Drive keyword search (keyword-gated)"]
    FB -->|hits| INJ
    FB -->|none| WEB["No KB → maybe web search"]
    INJ --> LLM["Gemini 3 / GPT — grounding contract:<br/>answer ONLY from chunks, cite [n], else ABSTAIN"]
```
...
> **Timezone (Eastern is canonical):** punches store absolute Firestore `Timestamp`s; the UI displays them anchored to **America/New_York (EST/EDT, DST-aware)** via `fmtTime`/`fmtDateTime` (which pass an explicit `timeZone`, so they're SSR-deterministic and identical for every viewer). The live **Punch clock** also shows a small "Your local time: …" reference under the big Eastern clock, gated by `useOffEastern()` (mount-resolved, off the SSR path) and only when the viewer's tz differs from Eastern; recorded rows stay Eastern-only. **Caveat:** day/week bucketing still uses the viewer's local calendar day (`startOfWeek`/`dayKey`), so a near-midnight punch can bucket on the adjacent day for off-ET viewers — acceptable for an ET-based org.
...
flowchart TD
    ...
    subgraph External["⚠️ Outside the GCP BAA — need own BAA or no PHI"]
        OAI["OpenAI<br/>chat/rewrite BACKUP (fallback) · Whisper<br/>→ needs an OpenAI BAA"]
...
**Routing (`PHI_SAFE_MODE`, default OFF — owner keeps OpenAI as a backup):** Vertex is PRIMARY everywhere; OpenAI is the automatic safety net. `chat.js` `/api/chat` — Gemini primary, **falls back to OpenAI `gpt-5-mini`** on error (default model `gemini-3-flash-preview`). `audio.js` `/api/rewrite` — Gemini `gemini-2.5-flash` primary, **OpenAI `gpt-4o-mini` backup**. `/api/transcribe` — **OpenAI Whisper by choice** (opt-in dictation; default is the browser Web Speech API). `/api/audio/speech` — `tts-1`, no live caller. The model picker is Gemini-only (OpenAI is an automatic backup, not a manual pick). **Strict mode** (`PHI_SAFE_MODE='true'`) flips all of this to Vertex-only (fallback retries `gemini-2.5-flash`, explicit OpenAI picks route to Vertex, fails closed).

**Top architectural gaps:** (1) **BAAs** — confirm the GCP BAA; because OpenAI is intentionally the chat/rewrite **backup** + Whisper engine, an **OpenAI BAA** is the path to cover those (or flip `PHI_SAFE_MODE='true'`). (2) **Server-side ID-token auth** — most function routes trust a body `userId` and `/api/admin/*` has none (see §8 ⚠️); verify the Firebase ID token on any PHI route. (3) **PHI-access audit log** — extend the `time_audit` immutability pattern to a `phi_access_log` (6-yr retention). Full P0/P1/P2 plan in [SOT §10.2](./SOURCE_OF_TRUTH.md#102-gaps--how-to-improve-prioritized).
