...
const fileToBase64 = <T extends {file?: File; base64: string; mimeType: string}>(
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      if (base64) {
        resolve({file, base64, mimeType: file.type} as T);
      } else {
...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  useEffect(() => {
    if (selectedModel?.id.includes('sora')) {
        // Sora models might only support 1080p or specific resolutions
        // Defaulting to 1080p to be safe as 720p caused issues
  ...
  // Derived state for model type
  const isSora = selectedModel?.id.includes('sora') ?? false;
  ...
  return (
    <div className="relative w-full">
      {isSettingsOpen && (
        <div className="absolute bottom-full left-0 right-0 mb-3 p-4 bg-white dark:bg-[#2c2c2e] rounded-xl border border-gray-200 dark:border-gray-700 shadow-2xl">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <CustomSelect
              ...
              icon={<RectangleStackIcon className="w-5 h-5 text-gray-500 dark:text-gray-400" />}
            ...
            <div>
              <CustomSelect
                ...
                icon={<TvIcon className="w-5 h-5 text-gray-500 dark:text-gray-400" />}
                disabled={isRefMode || isExtendMode}>
                {!isSora && <option value={Resolution.P720}>720p</option>}
                <option value={Resolution.P1080}>1080p</option>
              </CustomSelect>
              {resolution === Resolution.P1080 && (
                <p className="text-xs text-yellow-600 dark:text-yellow-400/80 mt-2">
                  1080p videos can't be extended.
                </p>
              )}
            </div>
            
            {/* Duration Slider - Only show if supported */}
            <div>
                ...
                <div className="relative flex items-center h-[42px]">
                    <input 
                        ...
                        disabled={!isSora} // Veo is fixed at 5s usually, Sora is 4-8s
                        className="w-full accent-indigo-500"
                    />
                </div>
                {!isSora && (
                    <p className="text-xs text-gray-500 mt-1">Fixed for this model</p>
                )}
            </div>
        ...
        <div className="flex items-end gap-2 bg-white dark:bg-[#1f1f1f] border border-gray-300 dark:border-gray-600 rounded-2xl p-2 shadow-lg focus-within:ring-2 focus-within:ring-indigo-500">
          <div className="relative" ref={modeSelectorRef}>
            <button
              ...
              className="flex shrink-0 items-center gap-2 px-3 py-2.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors"
          ...
          <textarea
            ...
            className="flex-grow bg-transparent focus:outline-none resize-none text-base text-gray-900 dark:text-gray-200 placeholder-gray-500 max-h-48 py-2"
            rows={1}
          />
          <button
            type="button"
            onClick={() => setIsSettingsOpen((prev) => !prev)}
            className={`p-2.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 ${isSettingsOpen ? 'bg-gray-200 dark:bg-gray-700 text-indigo-600 dark:text-white' : 'text-gray-500 dark:text-gray-300'}`}
